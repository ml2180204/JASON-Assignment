...
public class RRModelxxx extends GridWorldModel{
	...
	// Constructor method
	public RRModelxxx(int l, int w, int gl, int gw) {
			...
			for (int y = 0; y < ROWS; y++) {
				for (int x = 0; x < COLUMNS; x++) {
					for(Integer ps_head=0; ps_head<4; ps_head++) {
						...
						if(upObs(curr_location.x, curr_location.y, curr_head)) {
							if(upObs(x,y,ps_head)) {
								flag1=true;
							}
						} else if(downObs(curr_location.x, curr_location.y, curr_head)){
							if(downObs(x,y,ps_head)) {
								flag1=true;
							}
						} else if(leftObs(curr_location.x, curr_location.y, curr_head)) {
							if(leftObs(x,y,ps_head)) {
								flag1=true;
							}
						} else if(rightObs(curr_location.x, curr_location.y, curr_head)) {
		...
		if(!ps_square.isEmpty()) {
			for(int i=0, len1=ps_square.size(); i<len1;i++) {
				for(int j=0, len2=ps_square.get(i).getHeadList().size(); j<len2; j++) {
					...
					if(downObs(curr_location.x, curr_location.y, curr_head)
							&& !downObs(x,y,ps_head)) {
						flag2=true;
					}
					if(!downObs(curr_location.x, curr_location.y, curr_head)
					...
					if(leftObs(curr_location.x, curr_location.y, curr_head)
							&& !leftObs(x,y,ps_head)) {
						flag2=true;
					}
					if(!leftObs(curr_location.x, curr_location.y, curr_head)
					...
					if(rightObs(curr_location.x, curr_location.y, curr_head)
		...
		if(!upObs(x,y,ps_head)) {
			for(int i=0; i<ps_square.size();i++) {
				for(int j=0; j<ps_square.get(i).getHeadList().size();j++) {
					x = ps_square.get(i).getXCoordinate();
					y = ps_square.get(i).getYCoordinate();
					int h = ps_square.get(i).getHeadList().get(j);
					if(h%4==0) {
						if(!next_ps_square.contains(grid[x][y-1])) {
							next_ps_square.add(grid[x][y-1]);
			...
			for(int i=0; i<ps_square.size();i++) {
				for(int j=0; j<ps_square.get(i).getHeadList().size();j++) {
					...
					if(h%4==0) {
						...
						grid[x][y+1].addNextHead(s);
					} else if((h%4==1||h%4==-3)) {
						if(!next_ps_square.contains(grid[x-1][y])) {
							next_ps_square.add(grid[x-1][y]);
						}
						grid[x-1][y].addNextHead(s);
					} else if((h%4==-1||h%4==3)) {
						if(!next_ps_square.contains(grid[x+1][y])) {
							next_ps_square.add(grid[x+1][y]);
						}
